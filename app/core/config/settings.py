...
class Settings(BaseSettings):
    ...
    # ``images_default_model`` is the public model used when clients omit
    # ``model``; it must remain in the supported ``gpt-image-*`` family.
    images_host_model: str = "gpt-5.5"
    images_default_model: str = "gpt-image-2"
    images_max_partial_images: int = Field(default=3, ge=0, le=3)
    # NOTE: there is intentionally no ``images_max_n`` setting. The upstream
    # ``image_generation`` tool path accepts only a single image per call and
    # codex-lb does not yet implement client-side fan-out, so ``n > 1`` is
    # hard-rejected at the API boundary. The cap is lifted in the same change
    # that introduces fan-out.
