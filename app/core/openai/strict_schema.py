...
When ``strict: true`` is set on a ``json_schema`` response format, the
upstream API requires every object schema to opt out of extra properties and
requires all declared properties to be listed in ``required``. Rejecting those
violations locally preserves OpenAI-compatible ``invalid_json_schema`` errors
before any upstream request is opened.
"""
...
def _find_violation(
    ...
    if schema_type == "object" or "properties" in node:
        ...
        if is_json_mapping(properties):
            ...
            missing_required = [str(prop_name) for prop_name in properties if str(prop_name) not in required_set]
            if missing_required:
