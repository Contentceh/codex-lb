...
Status: [x] Completed as a readiness/evidence sprint; future 1.16 cutover remains blocked on official upstream release availability and requires separate operator approval.
...
Checklist:
- [x] Fetch upstream refs and record `git ls-remote --tags upstream 'refs/tags/v1.16*'` without assuming a tag exists.
- [x] Record current branch, HEAD, remotes, `git status --short --branch`, and whether the worktree contains non-doc changes.
- [x] Record Docker service names/images/status and health statuses for `/health/live` then `/health/ready` without printing secrets.
- [x] Record sanitized presence of image/runtime settings, including `CODEX_LB_IMAGES_DEFAULT_MODEL`, `CODEX_LB_IMAGES_HOST_MODEL`, and `CODEX_LB_IMAGES_MAX_PARTIAL_IMAGES`, without saving values.
- [x] If no upstream `v1.16*` ref exists, mark later candidate/cutover issues as blocked by release availability instead of inventing a target.
Completed commit: [x] 9d07f1d docs(upgrade): capture sprint 5 evidence
...
Checklist:
- [x] Compare `v1.15.0..upstream/main` and, once available, `v1.15.0..v1.16.0` with `git diff --stat`, `git log --oneline`, and targeted file lists.
- [x] Identify changes touching proxy routes, Responses/Chat/Image compatibility, request logs, usage/pricing, API keys, accounts/OAuth, Alembic migrations, Docker/compose, and frontend dashboard surfaces.
- [x] Use targeted `grep` and `head -n 50` for changed files before writing risk notes.
- [x] Record whether any upstream changes supersede local Sprint 1-4 patches or create likely conflicts.
- [x] Keep the artifact text-only and free of secrets, request bodies, generated images, or base64 payloads.
Completed commit: [x] 9d07f1d docs(upgrade): capture sprint 5 evidence
...
Checklist:
- [x] Compare the local image branch against `v1.15.0...upstream/pr/498` and latest upstream refs to identify duplicate, changed, and still-local image patches.
- [x] Review `/v1/images/generations`, `/v1/images/edits`, explicit `/v1/images/variations` unsupported behavior, schema validation, transport selection, request logs, usage/cost pricing, and API-key allowed-model policy.
- [x] Decide per patch chunk: drop because upstream includes it, rebase because upstream changed nearby code, or retain because it remains a local requirement.
- [x] Preserve the public `gpt-image-*` accounting/model-policy boundary and verify no public evidence requires `images_host_model`.
- [x] Document expected tests and linters for whichever path is selected.
Completed commit: [x] 9d07f1d docs(upgrade): capture sprint 5 evidence
...
Checklist:
- [x] Name the candidate branch pattern for a future upstream ref, for example `upgrade/from-1.15.0-images-to-1.16.0`.
- [x] Require a clean worktree or documented stash/patch backup before branch creation.
- [x] Define conflict-resolution priority: upstream 1.16 behavior first, then live-data preservation, then local PR #498 compatibility only where upstream lacks equivalent behavior.
- [x] Define when to stop and record a blocker instead of resolving ambiguous conflicts, especially around migrations, Docker volume paths, OAuth/account storage, and image accounting.
- [x] Confirm `uv.lock` changes are allowed only when caused by an explicit upstream dependency change and recorded in evidence.
Completed commit: [x] 9d07f1d docs(upgrade): capture sprint 5 evidence
...
Checklist:
- [x] Run migration/head checks against a non-live database fixture before touching the live Docker volume.
- [x] Run targeted backend regressions for migrations, proxy Responses/Chat, image routes, model listing, API keys, request logs, usage/pricing, and account compatibility.
- [x] Run `uvx ruff check` for touched backend paths and targeted tests.
- [x] Run `git diff --check`, `git diff --cached --check`, `git diff --exit-code -- uv.lock`, and `git diff --cached --exit-code -- uv.lock`.
- [x] Record failures with exact command, exit code, and redacted logs; do not proceed to live rehearsal while backend gates fail.
Completed commit: [x] 9d07f1d docs(upgrade): capture sprint 5 evidence
...
Checklist:
- [x] Review dashboard API calls for accounts, settings, API keys, request logs, usage summaries, and model options against any upstream 1.16 API changes.
- [x] Run `bun run lint`, `bun run typecheck`, `bun run test`, and `bun run build` from `frontend/`.
- [x] If `bun` or frontend dependencies are unavailable, record the missing-tool blocker and do not mark this issue complete.
- [x] Verify no frontend artifact contains secrets, live request bodies, or generated image/base64 data.
Completed commit: [x] 9d07f1d docs(upgrade): capture sprint 5 evidence
...
Checklist:
- [x] Document exact build, restart, and rollback commands for the observed `server` + `frontend` stack without using volume-destructive flags.
- [x] Require fresh pre-rehearsal backup location and current image/commit identifiers before any Docker restart.
- [x] Define live smoke order: `/health/live`, `/health/ready`, dashboard/API surface checks, minimal authenticated text proxy smoke, `/v1/models`, request logs/usage, then conservative image validation/live smokes only if needed.
- [x] Limit paid image generation to at most one non-streaming and one streaming `gpt-image-2` request unless the operator approves more.
- [x] Record rollback criteria and stop conditions for migration errors, missing accounts/settings, model-scope failures, public/private model leaks, or health regressions.
Completed commit: [x] 9d07f1d docs(upgrade): capture sprint 5 evidence
...
Checklist:
- [x] Summarize release availability, upstream delta map, PR #498 drop/rebase/retain decision, candidate branch policy, validation gates, and rollback plan.
- [x] Include `release_availability_gate=PASS|BLOCKED`, `delta_map_gate=PASS`, `patch_alignment_gate=PASS`, `validation_matrix_gate=PASS`, `rollback_plan_gate=PASS`, and `secret_scan=PASS` markers only after underlying artifacts exist.
- [x] Run a suppressed secret scan for Sprint 5 artifacts that never prints matched line contents.
- [x] Update roadmap Sprint 5 status to Completed only if exit criteria are met; otherwise mark the explicit upstream-release blocker and keep future cutover work planned.
- [x] Run `git diff --check`, `git diff --cached --check`, `git diff --exit-code -- uv.lock`, `git diff --cached --exit-code -- uv.lock`, and `git status --short --branch`.
Completed commit: [x] 9d07f1d docs(upgrade): capture sprint 5 evidence

## Sprint quality gates
- [x] Upstream `v1.16*` release availability is recorded with exact refs inspected, including an explicit blocker if no tag exists.
- [x] Current branch, commit, remotes, live Docker service names, compose strategy, and health endpoints are captured without secrets.
- [x] Delta map covers backend proxy, `/v1/images/*`, request logs, usage/pricing, API keys, accounts/OAuth, migrations, Docker/compose, and frontend dashboard surfaces.
- [x] PR #498-derived local patches have a documented drop/rebase/retain decision matrix before any candidate coding work begins.
- [x] Candidate branch policy preserves `localai_codex-lb-data`, existing accounts/settings/API keys, OAuth/encryption material, and current service names.
- [x] Backend candidate gates include targeted `pytest` plus `uvx ruff check` for touched paths.
- [x] Frontend candidate gates include lint, typecheck, tests, and build.
- [x] `git diff --check`, `git diff --cached --check`, and `uv.lock` clean checks pass before closure.
- [x] Live rehearsal/cutover remains blocked until a valid upstream ref, non-live validation, rollback notes, and operator approval exist.
- [x] Sprint 5 artifacts pass suppressed secret scan and contain no raw API keys, OAuth token JSON, `.env.local`, database dumps, encryption key material, full Authorization headers, generated binary images, or `b64_json`.
